...
2. `docs/elicitation_and_mutation.md` — the **Track 1 (spec elicitation)**
   and **Track 2 (spec mutation testing)** contributions, with method,
   numbers, and limitations. New since the original writeup.
3. `docs/comparison_methodology.md` — head-to-head vs TiCoder,
   nl2postcond, Kiro PBT, PRDJudge with the actual numbers.
4. `docs/related_work.md` — the survey of prior art.
5. `docs/track1_task_specs.md` — the original contribution claim writeup
   (validator + 6-invariant DSL).
6. `safe_scaffold/task_spec/` — the implementation; ~2400 LOC total,
   read order: `spec.py` → `invariants.py` → `validator.py` →
   `baselines.py` → `eval.py` → `metrics.py` → `ablation.py` →
   `elicitation.py` → `spec_mutation.py`.

## Try the elicitation + mutation pieces

```bash
# Spec mutation testing on the whole corpus (Track 2, no API key needed):
PYTHONPATH=. python3 -m safe_scaffold.cli mutate

# LLM-drafted spec from an NL intent + a directory of files (Track 1):
...
# Browser UI with four tabs (Validate · Draft · Compare · Mutate):
PYTHONPATH=. python3 demo_server.py
# → http://127.0.0.1:8765
```

## Lean 4 emission (Track 1 — literal "into Lean")
...
`Spec_<task_id>.lean` per TaskSpec.
